...
using System.Text;
using FixedPointy;
using PicoMoonSharp.Interpreter.Compatibility;

namespace PicoMoonSharp.Interpreter.Tree
{
	internal static class LexerUtils
	{
		public static Fix ParseNumber(Token T)
		{
			string txt = T.Text;
			double res;
        ...
        public static Fix ParseBinInteger(Token T)
        {
            string txt = T.Text;
            ...
            short res = Convert.ToInt16(txt.Substring(2), 2);

            return (Fix)res;
        }

        public static Fix ParseBinFloat(Token T)
        {
            string txt = T.Text;
            if ((txt.Length < 2) || (txt[0] != '0' && (char.ToUpper(txt[1]) != 'B')))
                throw new InternalErrorException("binary numbers must start with '0b' near '{0}'.", txt);
            ...
            ushort integer  = Convert.ToUInt16(parts[0], 2);
            ushort fraction = Convert.ToUInt16(parts[1], 2);

            int raw = integer << Fix.FractionalBits | fraction;

            return new Fix(raw);
        }

		public static Fix ParseHexInteger(Token T)
		{
			string txt = T.Text;
		...
		public static Fix ParseHexFloat(Token T)
		{
			string s = T.Text;
        ...
        public static bool CharIsBinDigit(char c)
        {
            return c == '0' || c == '1';
        }
