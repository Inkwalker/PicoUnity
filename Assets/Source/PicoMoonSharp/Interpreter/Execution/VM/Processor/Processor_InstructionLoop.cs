...
namespace PicoMoonSharp.Interpreter.Execution.VM
{
	sealed partial class Processor
	{
		...
		{
			Fix? v = m_ValueStack.Pop().ToScalar().CastToNumber();
			if (v.HasValue)
		...
		{
			Fix val  = m_ValueStack.Peek(0).Number;
			Fix step = m_ValueStack.Peek(1).Number;
			Fix stop = m_ValueStack.Peek(2).Number;
		...
		private int ExecAdd(Instruction i, int instructionPtr)
		{
			...
			Fix? rn = r.CastToNumber();
			Fix? ln = l.CastToNumber();
		...
		private int ExecPower(Instruction i, int instructionPtr)
		{
			...
			if (ln.HasValue && rn.HasValue)
			{
				m_ValueStack.Push(DynValue.NewNumber(FixMath.Pow(ln.Value, rn.Value)));
				return instructionPtr;
			}
			else
			{
